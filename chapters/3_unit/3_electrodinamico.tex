...
El campo magnético formado por la bobina fija es, en todo momento, proporcional al valor instantáneo de la intensidad que la recorre: \(i_A \propto B\). Así mismo, la intensidad en la bobina móvil es proporcional a la tensión \(i_u \propto u\)
...
\[
  C_m \propto B\cdot i_u \propto i_A \cdot u \propto p
\]
...
\[
  P_\text{med} = P=V\cdot I\cdot \cos(\varphi) \propto (C_m)_\text{med}
\]
...
\[
  (C_m)_\text{med} = C_d \propto \alpha
\]
\begin{figure}[!ht]
  \centering
...
\textit{Nota conceptual:} Si se usa como amperímetro (las dos bobinas en serie, \(I_F=I_M=I\)), la escala es cuadrática (\(\alpha \propto I^2\)). Si se usa como vatímetro, es lineal respecto a la potencia.
...
\section*{Notas Conceptuales: Interpretación Física}
\addcontentsline{toc}{section}{Notas Conceptuales: Interpretación Física}

Para cerrar el tema, es fundamental entender intuitivamente la diferencia entre este instrumento y los de imán permanente (PMMC), y por qué las escalas cambian según el uso.

\subsection*{1. ¿Por qué la escala es cuadrática en Amperímetros y Voltímetros?}
En un instrumento de imán permanente, el campo magnético \(B\) es fijo (lo genera el imán). La fuerza solo depende de la corriente de la bobina móvil (\(F \propto I\)).

En el \textbf{electrodinámico}, no hay imanes. El campo \(B\) lo genera la corriente que pasa por la bobina fija. Esto crea un ``efecto doble'':
\begin{enumerate}
    \item Si aumenta la corriente, aumenta el campo magnético (\(B \propto I\)).
    \item Si aumenta la corriente, aumenta la fuerza en la bobina móvil.
\end{enumerate}

Matemáticamente, la deflexión \(\alpha\) responde al producto de ambas corrientes (que en este caso son la misma):
\[
    \alpha \propto B_{\text{fija}} \cdot I_{\text{móvil}} \propto I \cdot I = I^2
\]
Esto explica por qué la aguja apenas se mueve con corrientes bajas y se dispara con corrientes altas. Además, como el cuadrado de un número negativo es positivo (\((-I) \cdot (-I) = I^2\)), el instrumento siempre deflecta en el mismo sentido, permitiendo medir el valor eficaz (RMS) tanto en CC como en CA.

\subsection*{2. El ``misterio'' de la Inductancia Mutua (\(M\))}
Aparece el término \(\frac{dM}{d\alpha}\) en todas las ecuaciones. ¿Qué significa físicamente?

Podemos ver a \(M\) como el ``grado de acoplamiento'' entre las dos bobinas.
\begin{itemize}
    \item Cuando las bobinas están paralelas, el acoplamiento es máximo.
    \item La derivada \(\frac{dM}{d\alpha}\) representa la ``intención'' del sistema de buscar esa posición de máximo acoplamiento.
\end{itemize}
La aguja se mueve simplemente porque las leyes del electromagnetismo buscan que la energía magnética del sistema sea mínima (o el flujo concatenado sea máximo), alineando los campos de la bobina fija y la móvil.

\subsection*{3. ¿Por qué el Vatímetro SÍ es lineal?}
A diferencia del amperímetro, en el vatímetro separamos las funciones:
\begin{itemize}
    \item La \textbf{Bobina Fija} ve solo la Corriente (\(I\)).
    \item La \textbf{Bobina Móvil} ve solo la Tensión (\(V\)).
\end{itemize}
La aguja responde al producto de estos dos campos independientes:
\[
    \alpha \propto I_{\text{fija}} \cdot I_{\text{móvil}} \propto I \cdot V = P
\]
Como la deflexión es proporcional a la potencia \(P\) (y no al cuadrado de una sola magnitud), la escala resulta \textbf{lineal} y uniforme.
